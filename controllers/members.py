...
def assign_photos(story_list):
    ...
    lst = [rec for rec in lst if rec.photo_path]
    for photo in lst:
...
def get_member_photos(member_id):
    ...
    uns = [r['photo_date'] for r in lst]
    sor = [r['photo_date'] for r in result]
    comment(f"------=====-----slides unsorted: {uns[:25]}")
    comment(f"*************slides sorted: {sor[:25]}")
    return result
