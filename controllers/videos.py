...
def get_video_sample(vars):
    ...
    lst1 = [dict(src=rec.src,video_id=rec.id, name=rec.name, thumbnail_src=rec.thumbnail_src) for rec in lst1]
    q2 = q & (db.TblVideos.touch_time == NO_DATE)
    lst2 = db(q2).select(limitby=(0, 200))
    lst2 = [dict(src=rec.src,video_id=rec.id, name=rec.name, thumbnail_src=rec.thumbnail_src) for rec in lst2]
    if len(lst2) > 10:
