...
from docs_support import save_uploaded_doc, doc_url, doc_jpg_url, \
    ...
    restore_doc_thumbnail, confirm_doc_thumbnail, copy_doc_date_to_segments
...
def get_doc_segment_info(vars):
    ...
    doc_src=doc_segment_url(story_id, rec)
    doc_segment_thumbnail_url = doc_segment_jpg_url(story_id, None)
    ...
    return dict(
        ...
        doc_segment_thumbnail_url=doc_segment_thumbnail_url
    )
...
def convert_story_ids(vars):
    ...
    doc_ids = [int(di) for di in vars.doc_ids] if vars.doc_ids else []
    if segment_id:
...
@serve_json
def copy_doc_date_to_doc_segments(vars):
    doc_id = int(vars.doc_id)
    copy_doc_date_to_segments(doc_id)
    return dict()
